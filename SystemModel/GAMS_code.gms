$TITLE: SWAMPS : Systems model in Wetlands to Allocate water and Manage Plant Spread

$Ontext

Created by Omar Alminagorta, Utah State University.
...
*For Vegetation response
     Activate the Eq. :   (VegResp(mn)*InitCV_iv(yr,mn,dy,wu))
...
EqMassBal_WU(yr,mn,dy,wu)..
        ...
        -(Evapo(yr,mn,dy,wu)$(ord(mn) GT 1))
         =E= S(yr,mn,dy,wu)
            - (initStor(yr,mn,dy,wu)$(ord(yr) EQ 1 and ord(mn) EQ 1))
...
EqRemovedCV(yr,mn,dy,wu)..
                          RemovedCV(yr,mn,dy,wu)=e= ((InitCV_iv(yr,mn,dy,wu)
                               $(ord(yr) EQ 1 and ord(mn) EQ 1)
...
* A = minimum asymptote, this can be thought of as the response value at 0 standard concentration.
* B = Hill slopeThe Hill Slope or slope factor refers to the steepness of the curve. It could either be positive or negative. As the absolute value of the Hill slope increases, so does the steepness of the curve.
* C = inflection pointThe inflection point is defined as the point on the curve where the curvature changes direction or signs. This can be better explained if you can imagine the concavity of a sigmoidal curve. The inflection point is where the curve changes from being concave upwards to concave downwards
* D = maximum asymptote, this can be thought of as the response value for infinite standard concentration.
...
OPTION NLP=CONOPT;
Solve Nov31  MAXIMIZING Obj2 USING NLP ;
OPTION Savepoint=1
...
*$ontext
*Sensitivity Analysis considering the Inflow
*Probably the values of Inflow is to general (becarful, "wu"-"jn")

*step 1 - setup scenarios
set
ScenQ /Base,Dry,Wet/ ;

Table Variation3 (yr,mn,dy,ScenQ)  Different Scenarios of inflow   (Ha-m-month)

                   Base          Dry             Wet
2010.1.1        6433.07        5453.17        20902.56
2010.2.1        7706.71        5520.75        12960.98
2010.3.1        10140.32       7117.94        20204.80
2010.4.1        9835.23        4681.28        28191.89
2010.5.1        10777.41        544.56        42222.27
2010.6.1        8734.27         569.56        36250.91
2010.7.1        646.19          613.58        10678.81
2010.8.1        648.46          418.66        11952.99
2010.9.1        1446.66         456.53        13637.21
2010.10.1       6012.90         725.07        18642.42
2010.11.1       6594.74        4891.93        20184.25
2010.12.1       6782.71        4847.94        18088.76


parameter
report3(*,*)    report results

*step 2 save data

record3 (yr,mn,dy,i) recording results;
*record3 (yr,mn,dy,jn) = jn_ts_data(yr,mn,dy,"Inflow",jn);
record3 (yr,mn,dy,jn) =  Inflow(yr,mn,dy,"In") ;


Loop (ScenQ,
*Step 3 Reset values
Inflow(yr,mn,dy,"In")= record3(yr,mn,dy,"In");
*Inflow(yr,mn,dy,i) = record3(yr,mn,dy,jn);
*Step 4 - Set new values
*^Inflow(yr,mn,dy,"In") = Variation3(yr,mn,dy,ScenQ);
Inflow(yr,mn,dy,"In")$Variation3(yr,mn,dy,ScenQ)= Variation3(yr,mn,dy,ScenQ);

Solve Nov30  MAXIMIZING Obj USING NLP ;
report3("Obj",ScenQ) = Obj.L ;
*report3("Inflow",ScenQ) = Inflow.L (yr,mn,dy,jn) ;
